   ...
   -->                          <td>Batsman</td>
                          ...
                          <?php 
                              ...
                              if((preg_match($Bat, $tossNchoise) and preg_match($teamxpat, $tossNchoise)) or (preg_match($Bowl, $tossNchoise) and preg_match($teamypat, $tossNchoise))){
                                ...
                                $str1="SELECT * from `batting_team` where match_id=$match_id and team_name='$teambat'";
                                $table1=mysqli_query($con,$str1);
                                $str2="SELECT * from `bowling_team` where match_id=$match_id and team_name='$teambowl'";
                                $table2=mysqli_query($con,$str2);
                          ...
                          while($batsman=mysqli_fetch_assoc($table1) and $bowler=mysqli_fetch_assoc($table2)){
                           ...
                           echo "<td style='display:none;'><input type='number' name='idb[]' value=".$bowler['id']."></td>";
                            ...
                            $o=(intdiv($bowler['overs'],6))+(($bowler['overs']%6)/10);
                              echo "<td><input type='number' name='overs[]' placeholder=".$o."></td>";
                      ...
                      echo "<tr class='rows'>";
                          ...
                          <tr class='rows'>
                            ...
                            <?php  
                                ...
                                $wickts=$row['wickts'];
                                $sndtotal_runs=$row['total_runs'];
                                mysqli_next_result($con);

                                $str5="CALL `calculate_score`($match_id, '$teambat')";
                                $sql=mysqli_query($con,$str5);
                                $row=mysqli_fetch_assoc($sql);
                                $fsttotal_runs=$row['total_runs'];
                                mysqli_next_result($con);
                              ?>


                            <td style="background: linear-gradient(-135deg, #000a02de, #1e7e34);color: white;"><?php echo "$matchovers"; ?></td>
                          	<td style="background: linear-gradient(-135deg, #000a02de, #1e7e34);color: white;"><?php  echo "$sndtotal_runs"; ?></td>
                          	<td style="background: linear-gradient(-135deg, #000a02de, #1e7e34);color: white;"><?php  echo "$wickts"; ?></td>
                          	
                          	
                          	<td style="border-radius:10px;"><input type="submit" name="prev" value="PREV" style="background: linear-gradient(-135deg, #7d31e8ed, #4a1c93);color: white;border-radius:10px;"></td>
                            
                            
                          </tr>
      </table>
      </form> 
<?php if(isset($_POST['prev'])){ echo "<script> location.href='1stinnings.php'; </script>"; } ?>
<?php 
        if(($sndtotal_runs==$fsttotal_runs) and (($matchovers==$match_overs) or $wickts==10)  ){
                 mysqli_query($con,"UPDATE matches_schedule SET `winner` ='draw' WHERE `matches_schedule`.`match_id` = $match_id");                           
                 mysqli_query($con,"UPDATE matches_schedule set status=3 where match_id='$match_id'");
                 mysqli_query($con,"UPDATE `team_points` set draws=draws+1 where team_name='$teambat'");
                 mysqli_query($con,"UPDATE `team_points` set draws=draws+1 where team_name='$teambowl'");
                echo "<script>alert('match draw');</script>";
         }
        if($sndtotal_runs>$fsttotal_runs){
               mysqli_query($con,"UPDATE matches_schedule set winner='$teambat' where match_id=$match_id");
               mysqli_query($con,"UPDATE matches_schedule set status=3 where match_id='$match_id'");
               mysqli_query($con,"UPDATE `team_points` set wins=wins+1 where team_name='$teambat'");
               mysqli_query($con,"UPDATE `team_points` set loss=loss+1 where team_name='$teambowl'");
               echo "<script>alert('$teambat won');</script>";
         }
        if(($sndtotal_runs<$fsttotal_runs) and (($matchovers==$match_overs) or $wickts==10)  ){
           mysqli_query($con,"UPDATE matches_schedule set winner='$teambowl' where match_id=$match_id");
           mysqli_query($con,"UPDATE matches_schedule set status=3 where match_id='$match_id'");
           mysqli_query($con,"UPDATE `team_points` set wins=wins+1 where team_name='$teambowl'");
           mysqli_query($con,"UPDATE `team_points` set loss=loss+1 where team_name='$teambat'");
           echo "<script>alert('$teambowl won');</script>";
        } 
 ?>
...
<?php 
 ...
 if(isset($_POST['update'])){
        ...
        if(($matchovers<$match_overs) and ($wickts<10) and ($sndtotal_runs<=$fsttotal_runs)){
                for($j=0;$j<$countb;$j++){
                  $idb=$_POST['idb'][$j];
                  $overs=$_POST['overs'][$j];
                  $runsgiven=$_POST['runsgiven'][$j];
                  $wicks=$_POST['wicks'][$j];
                  $extras=$_POST['extras'][$j];
                  if(isset($overs)){
                    $ob=($overs-(int)$overs)*10+(int)$overs*6;
                    $str="UPDATE `bowling_team` SET overs=$ob where match_id=$match_id and team_name='$teambowl' and id=$idb ";  $sql=mysqli_query($con,$str);
                  }
                  if(isset($runsgiven)){
        ...
        echo "<meta http-equiv='refresh' content='0' url='2ndinnings.php'>";       
 }
 ?>
